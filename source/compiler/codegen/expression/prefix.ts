...
import { LinearType, OperandType, SolidType } from "~/compiler/codegen/expression/type.ts";
...
function CompileReturn(ctx: Context, type: OperandType, prefix: Syntax.Term_Expr_prefix) {
	if (!(type instanceof IntrinsicValue)) Panic(
		`${colors.red("Error")}: Cannot apply prefix operation to non-variable\n`, {
		path: ctx.file.path, name: ctx.file.name, ref: prefix.ref
	});
	...
	if (type instanceof LinearType) {
		ResolveLinearType(ctx, type, prefix.ref);
		type = type.type;
	}
